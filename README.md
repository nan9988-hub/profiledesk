...
- 业务站与账户支持右键编辑；业务站可修改名称、首页、LOGO和折叠背景色，账户右键直接进入环境配置。
- 顶部声音按钮按账户独立保存静音状态，切换账户时同步显示该账户的声音状态。
- 批量启动、停止、刷新、清缓存和创建快照。
- 账户行、环境配置底部和批量操作均可删除账户；业务站右键编辑弹窗也可连同组内账户删除，并由主进程二次确认。
- 删除后立即停止目标账户、清除浏览数据并置灰禁用，不强制重启其他正在使用的窗口；下次启动会在任何账户恢复前删除Profile、下载、快照、凭据和账户记录，避免Windows的`EBUSY`文件占用错误。
